...
import { getCountriesByLang2, getCountriesByName } from '../../../../helpers/selector'

function classNames(...classes) {
    return classes.filter(Boolean).join(' ')
}

export const SelectorEdit = ({ bg, cty }) => {

    const countries = getCountriesByLang2(countriesEsp, countriesEng, cty);
    const [selected, setSelected] = useState(getCountriesByName(countries, cty));
                        ...
                        >
                            <Listbox.Options className="absolute z-10 mt-2 w-full bg-white shadow-lg max-h-56 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                                {countries.map((country) => (
                                    ...
                                    >
                                        {({ selected, active }) => (
                                            <div onClick={() => localStorage.setItem('countryEdit', country.value)}>
                                                <div className="flex items-center" >
                                                    <img src={country.avatar} alt="" className="flex-shrink-0 h-6 w-6 rounded-full" />
